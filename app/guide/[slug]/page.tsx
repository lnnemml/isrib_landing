...
export async function generateStaticParams() {
  return getAllSlugs()
    .filter(s => s.cluster === 'guide')
    .map(s => ({ slug: s.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { frontmatter } = getArticle('guide', params.slug)
  const canonical = `${BASE_URL}/guide/${params.slug}`
  return {
...
export default function GuidePage({ params }: PageProps) {
  const { frontmatter, content } = getArticle('guide', params.slug)
  ...
  return (
    <>
      ...
      <ArticleLayout
        ...
        toc={toc}
        relatedArticles={
          <RelatedArticles slugs={frontmatter.relatedSlugs} currentCluster="guide" />
        }
      >
        <MDXRemote
          source={content}
