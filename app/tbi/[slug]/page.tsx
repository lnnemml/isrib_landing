...
export async function generateStaticParams() {
  return getAllSlugs()
    .filter(s => s.cluster === 'tbi')
    .map(s => ({ slug: s.slug }))
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { frontmatter } = getArticle('tbi', params.slug)
  const canonical = `${BASE_URL}/tbi/${params.slug}`
  return {
...
export default function TbiPage({ params }: PageProps) {
  const { frontmatter, content } = getArticle('tbi', params.slug)
  ...
  return (
    <>
      ...
      <ArticleLayout
        ...
        toc={toc}
        relatedArticles={
          <RelatedArticles slugs={frontmatter.relatedSlugs} currentCluster="tbi" />
        }
      >
        <MDXRemote
          source={content}
